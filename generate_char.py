...
# 20221102
#--out_dir E:/ocr/dataset --font_dir ./fonts/cn --width 40 --height 40 --margin 0 --rotate 0 --rotate_step 5 --need_aug

class dataAugmentation(object):
    def __init__(self,noise=False,dilate=True,erode=True,affine=False):
        self.noise = noise      #噪点
        self.dilate = dilate    #变细 腐蚀与膨胀( Erosion 与 Dilation)
        self.erode = erode      #加粗
        self.affine = affine    #仿射变换
    ...
    def add_erode(cls,img):
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(1, 2))#胖的横
    ...
    def add_dilate(cls,img):
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 1))#瘦的竖
        ...
        # img2 = cv2.dilate(img, kernel)
        #
        # kernel2 = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 2))  # 胖的横
        # img3 = cv2.erode(img2, kernel2)
        # return img3



    @classmethod
    def add_affine(cls,img):
        # 要有中心坐标、旋转角度、缩放系数
        ...
        dst4 = np.float32([[50, 50], [200, 40], [50, 210]])  # 往下歪
        M = cv2.getAffineTransform(src3, dst4)
        dst2 = cv2.warpAffine(img, M, (nW, nH), borderValue=(255, 255, 255))  # 画布大小乘以2

        return dst2

    @classmethod
    def add_affine2(cls,img):
        # 要有中心坐标、旋转角度、缩放系数
        h, w = image.shape[:2]  # 输入(H,W,C)，取 H，W 的值
        center = (w // 2, h // 2)  # 绕图片中心进行旋转
        angle = 5  # 旋转角度
        ...
        # 2 仿射变换
        src3 = np.float32([[50, 50], [200, 50], [50, 200]])  # 获取原图像三个角坐标
        dst4 = np.float32([[50, 50], [200, 60], [50, 190]])  # 往上歪
        M = cv2.getAffineTransform(src3, dst4)
        dst2 = cv2.warpAffine(img, M, (nW, nH), borderValue=(255, 255, 255))  # 画布大小乘以2
    ...
    def do(self,img_list=[]):
        ...
        for i in range(len(img_list)):
            ...
            if self.erode:
                ...
                # aug_list.append(im)
                aug_list.append(self.add_erode(im))
            if self.noise:
                im = self.add_noise(im)
                aug_list.append(im)
            if self.dilate:
                aug_list.append(self.add_dilate(im))
            if self.affine:
                aug_list.append(self.add_affine(im))
                #aug_list.append(self.add_affine2(im))
                #aug_list.append(self.image_rotate_2(im))
        return aug_list
...
class Font2Image(object):
    ...
    def do(self, font_path, char, rotate=0):
        ...
        draw.text((0, 0), char, (0, 0, 0),
                  font=font)
        if rotate != 0:
            img = img.rotate(rotate,fillcolor=(255, 255, 255))
        ...
        if sum_val > 2:
            np_img = np.asarray(data2, dtype='uint8')
            np_img = np_img[:, 0]
...
def get_label_dict():
    lang_dict = load_dict(
        os.path.join(os.path.dirname(__file__), "fonts", "cs.txt")
    )
...
python gen_printed_char2.py --out_dir ./dataset \
    ...
    train_images_dir2 = "E:\\ocr\\dataset\\train2"
    if os.path.isdir(train_images_dir2):
        shutil.rmtree(train_images_dir2)
    os.makedirs(train_images_dir2)
    ...
    for (char, value) in lang_chars.items():  # 外层循环是字
        ...
        for i in range(len(image_list)):
            ...
            fileName2 = char + "_" + fileName
            path_image = os.path.join(train_images_dir, fileName2)

            image2 = Image.fromarray(img)
            image2.save(os.path.join(train_images_dir, fileName2))
            count += 1
